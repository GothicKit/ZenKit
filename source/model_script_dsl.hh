...
namespace phoenix::mds {
	// TODO: case folding for everything
	...
	static constexpr auto ws = LEXY_LIT("/") >> dsl::until(dsl::newline) | dsl::ascii::space | LEXY_ASCII_ONE_OF("()");
	...
	struct dsl_event_camera_tremor {
		static constexpr auto whitespace = ws;
		static constexpr auto rule = (dsl::p<dsl_integer> + dsl::p<dsl_integer> + dsl::p<dsl_integer> +
		                              dsl::p<dsl_integer> + dsl::p<dsl_integer>);
	...
	struct dsl_event_pfx_stop {
		static constexpr auto whitespace = ws;
		static constexpr auto rule = (dsl::p<dsl_integer> + dsl::p<dsl_integer>);
	...
	struct dsl_event_sfx_grnd {
		static constexpr auto whitespace = ws;
		static constexpr auto rule = (dsl::p<dsl_integer> + dsl::p<string_without_closing_quote>);
	...
	struct dsl_event_sfx {
		static constexpr auto whitespace = ws;
		static constexpr auto rule = dsl::p<dsl_integer> + dsl::p<dsl_string> +
		    dsl::opt((dsl::lit_c<'r'> | dsl::lit_c<'R'>) >> (dsl::lit_c<':'> + dsl::p<dsl_float>) ) +
		    dsl::opt(LEXY_LIT("EMPTY_SLOT") | LEXY_LIT("EMTPY_SLOT"));
	...
	struct dsl_ani_comb {
		...
		static constexpr auto rule =
		    (dsl::p<dsl_string> + dsl::p<dsl_integer> + dsl::p<dsl_string> + dsl::p<dsl_float> + dsl::p<dsl_float> +
		     dsl::p<dsl_ani_flags> + dsl::p<dsl_string> +
		     dsl::p<dsl_integer>) +dsl::if_(dsl::lit_c<'{'> >> (dsl::while_(-LEXY_ASCII_ONE_OF("}")) +
		                                                        dsl::lit_c<'}'>) ); // TODO: Ani events on comb?
	...
	struct dsl_ani_disable {
		static constexpr auto whitespace = ws;
		static constexpr auto rule = (dsl::p<dsl_string>);
		static constexpr auto value = lexy::forward<std::string>;
	};

	struct dsl_model_tag {
		static constexpr auto whitespace = ws;
		static constexpr auto rule = (LEXY_LIT("\"DEF_HIT_LIMB\"") + dsl::p<dsl_string>);
		static constexpr auto value =
	...
	struct dsl_ani_enum {
		...
		static constexpr auto rule = dsl::lit_c<'{'> +
		    dsl::opt(dsl::list(
		        ...
		        LEXY_LIT("AniDisable") >> dsl::p<dsl_ani_disable> | LEXY_LIT("aniComb") >> dsl::p<dsl_ani_comb> |
		        LEXY_LIT("aniDisable") >> dsl::p<dsl_ani_disable> | LEXY_LIT("ani") >> dsl::p<dsl_ani> |
		        LEXY_LIT("modelTag") >> dsl::p<dsl_model_tag>)) +
		    dsl::opt(dsl::lit_c<'}'>);
	...
	struct dsl_skeleton {
		static constexpr auto whitespace = ws;
		static constexpr auto rule = (dsl::p<dsl_string> + dsl::opt(LEXY_LIT("DONT_USE_MESH")));
	...
	struct d_script {
		static constexpr auto whitespace = ws;
		static constexpr auto rule = LEXY_LIT("Model") + (dsl::p<dsl_string>) +dsl::p<dsl_script_body>;
