...
namespace phoenix::vobs {
	void zone_music::parse(zone_music& obj, std::unique_ptr<archive_reader>& ctx, game_version version) {
	...
	void zone_far_plane::parse(zone_far_plane& obj, std::unique_ptr<archive_reader>& ctx, game_version version) {
		vob::parse(obj, ctx, version);
		obj.vob_far_plane_z = ctx->read_float();        // vobFarPlaneZ
		obj.inner_range_percentage = ctx->read_float(); // innerRangePerc
	}

	void zone_fog::parse(zone_fog& obj, std::unique_ptr<archive_reader>& ctx, game_version version) {
		vob::parse(obj, ctx, version);
