...
namespace phoenix::vobs {
	void mob::parse(mob& obj, std::unique_ptr<archive_reader>& ctx, game_version version) {
	...
	void mob_inter::parse(mob_inter& obj, std::unique_ptr<archive_reader>& ctx, game_version version) {
	...
	void mob_container::parse(mob_container& obj, std::unique_ptr<archive_reader>& ctx, game_version version) {
	...
	void mob_door::parse(mob_door& obj, std::unique_ptr<archive_reader>& ctx, game_version version) {
		mob_inter::parse(obj, ctx, version);
		obj.locked = ctx->read_bool();        // locked
		obj.key = ctx->read_string();         // keyInstance
		obj.pick_string = ctx->read_string(); // pickLockStr
	...
	void mob_fire::parse(mob_fire& obj, std::unique_ptr<archive_reader>& ctx, game_version version) {
		mob_inter::parse(obj, ctx, version);
