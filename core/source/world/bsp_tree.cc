...
namespace phoenix {
	...
	static void _parse_bsp_nodes(reader& in, std::vector<bsp_node>& nodes, std::vector<u32>& indices, u32 version, s32 parent_index, bool leaf = false) {
	...
	bsp_tree bsp_tree::parse(reader& in) {
		...
		do {
			...
			if (in.tell() != end) {
				fmt::print(stderr, "warning: bsp tree: not all data or too much data consumed from section 0x{:X}\n", chunk);
			}
