...
phoenix::reader phoenix::reader::from(const std::string& path) {
	...
	if (fd < 0) {
		throw io_error("failed to open \"" + path + "\": " + std::strerror(errno));
	}

	struct stat st {};
	if (fstat(fd, &st) != 0) {
		throw io_error("failed to stat \"" + path + "\": " + std::strerror(errno));
	}
	if (S_ISDIR(st.st_mode)) {
		throw io_error("refusing to open directory \"" + path + "\"");
	}

	void* buffer = mmap(nullptr, st.st_size, PROT_READ, MAP_SHARED, fd, 0);
	if (buffer == MAP_FAILED) {
		throw io_error("failed to mmap \"" + path + "\": " + std::strerror(errno));
	}
	close(fd);
...
namespace phoenix {
	...
	void reader::seek(u64 offset) {
		if (offset > _m_buffer.size()) {
			throw io_error(
			    fmt::format("reader: refusing to seek to {} (max. {}): out-of-range", offset, _m_buffer.size()));
		}
