...
TEST_SUITE("script") {
	TEST_CASE("compiled daedalus script symbols are read correctly") {
		...
		CHECK(class_symbol->type() == phoenix::daedalus::dt_class);
		...
		CHECK(member_symbol->type() == phoenix::daedalus::dt_string);
		...
		CHECK(prototype_symbol->type() == phoenix::daedalus::dt_prototype);
		...
		CHECK(instance_symbol->type() == phoenix::daedalus::dt_instance);
		...
		CHECK(function_symbol->type() == phoenix::daedalus::dt_function);
		CHECK(function_symbol->has_return());
		CHECK(function_symbol->rtype() == phoenix::daedalus::dt_integer);
		// CHECK(function_symbol->parent() == -1); FIXME

		CHECK(external_symbol->name() == "UPDATE_CHOICEBOX");
		CHECK(external_symbol->count() == 1);
		CHECK(external_symbol->type() == phoenix::daedalus::dt_function);
	...
	TEST_CASE("compiled daedalus script instructions are read correctly") {
		...
		phoenix::daedalus::instruction ops[10] = {
		    phoenix::daedalus::instruction {.op = phoenix::daedalus::op_call, .address = 236},
		    phoenix::daedalus::instruction {.op = phoenix::daedalus::op_push_var, .symbol = 10},
		    phoenix::daedalus::instruction {.op = phoenix::daedalus::op_push_var, .symbol = 119},
		    phoenix::daedalus::instruction {.op = phoenix::daedalus::op_assign_string},
		    phoenix::daedalus::instruction {.op = phoenix::daedalus::op_push_var, .symbol = 426},
		    phoenix::daedalus::instruction {.op = phoenix::daedalus::op_push_var, .symbol = 126},
		    phoenix::daedalus::instruction {.op = phoenix::daedalus::op_assign_string},
		    phoenix::daedalus::instruction {.op = phoenix::daedalus::op_push_var, .symbol = 427},
		    phoenix::daedalus::instruction {.op = phoenix::daedalus::op_push_var, .symbol = 128},
		    phoenix::daedalus::instruction {.op = phoenix::daedalus::op_assign_string, .address = 0},
		};
